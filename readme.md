...
```
pnpm install  # nodeのモジュールのインストール
...
Vercelへのデプロイ時は以下が必要です。

## wasmをビルドして、 `public/wasm` 以下のファイルをGitの管理下に置く

`public/wasm` 以下をリポジトリの管理下に置くため、 `.gitignore` を削除してください。

## middleware.tsのuser名・パスワードを変更する

認証用のミドルウェアについて、user名・パスワードを変更してください。Edge Middlewareという技術を使っているそうです。
...
新しくプロジェクトを作ります。その際、Build Commandを以下のように変更してください。

```sh
tsc & vite build
```

また、URLも他の人とバッティングしないよう変えておいた方が良いかもしれません。
...
- `problem.html` : AtCoderの問題文をコピペしたテキストファイル
...
1. `problem.html` と `tools/src/` が配置されているかチェック
...
ビルド完了後は `pnpm dev` でビジュアライザを起動して動作を確認してください。
